...
parser.add_argument('--opp_strat', type=str, required=True, help='opponent strategy - required, options: Random, TFT, AD, AC, LLM ')
...
parser.add_argument('--moral_type', type=str, required=False, help='moral player type De or Ut - optional, default Ut')
...
moral_type = args.moral_type if args.moral_type else 'Ut'
print('for PART3, running moral type', moral_type)
#OPTION += moral_type #this is done in the shell script in this case 
...
if CD_tokens == 'unused': 
    ...
    seq_N = 2
    gen_seq_freq = None
else: #is generating random strings to represent C&D - NOTE this is probably now broken
...
if model_id == 'gpt2': 
    ...
    print(f'successfully instantiated AutoModelForCausalLMWithValueHead agentM (quantised & using LoRA, rank {LoRA_rank})')
print_cuda_mem_usage(device, episode=0, toprint='  MEM usage after first model loading: ')



model2 = AutoModelForCausalLMWithValueHead.from_pretrained(pretrained_model_name_or_path=f"{HF_HOME}/{model_name}",
                                                            attn_implementation='eager',
                                                            device_map={"": 0},
                                                            is_trainable=True,
                                                            peft_config=lora_config,
...
print(f'successfully instantiated AutoModelForCausalLMWithValueHead agentO (quantised & using LoRA, rank {LoRA_rank})')
print_cuda_mem_usage(device, episode=0, toprint='  MEM usage after second agentO model loading: ')
...
if CD_tokens not in ['action12', 'unused']:
...
ppo_trainer = PPOTrainer(config, model, ref_model, tokenizer=tokenizer)
...
if opponent_strategy == 'LLM':
    ppo_trainer2 = PPOTrainer(config, model2, ref_model, tokenizer=tokenizer) 
    ppo_trainer2.config.reward_model = None #overwrite default sentiment analysis reward model
    ppo_trainer2.config.query_dataset = None #overwrite default sentiment analysis dataset
    print('successfully instantiated ppo_trainer with model & ref_model')
...
####################################
if do_PART2: 
    results_df = pd.DataFrame(columns=['FineTuning_PART', 'episode', 'iteration', 'C_str', 'D_str', 'prev_move_M', 'prev_move_O', 'action_M', 'action_O', 'reward_M', 'reward_O'])
    results_idx = -1
    ...
    for episode in range(num_episodes): 
        ...
        batch_queries2 = []
        batch_input_tensors_squeezed2 = []
        batch_responses_squeezed2 = []
        batch_responses_cpu2 = []
        batch_responses_text2 = []
        batch_responses_text_withspecial2 = []
        batch_opp_moves2 = []
        batch_rewards2 = []
        ...
        for batch_step in range(max_steps_per_episode):
            ...
            query_text1 = generate_IPD_query(prev_move_M, prev_move_opp, model_name, C_placeholder, D_placeholder, RN_stream_1, RN_stream_2)
            if episode == 0: 
                print('query_text1 (before insert_CD_symbols): \n', query_text1)
            ...
            response_tensor1 = respond_to_batch(model, query_input_tensors1, txt_len=gen_len, top_k = 0, top_p = top_p)
            ...
            print_cuda_mem_usage(device, episode, toprint=f'  MEM usage during batch step {batch_step}, after decoding response from model1: ')
            response_text_withspecial1 = tokenizer.decode(response_tensor1.squeeze(0), skip_special_tokens=False)
            print_cuda_mem_usage(device, episode, toprint=f'  MEM usage during batch step {batch_step}, after decoding response from model1 with special tokens: ')
            action_playerM = get_action_token_v3(response_text1) #text form 
            if episode == 0:
                print('response_tensor: ', response_tensor1)
                print('response_text_withspecialtokens, squeeze: ', response_text_withspecial1)

                print('action_playerM (skip special tokens & strip): ', action_playerM, '\n')
            ...
            elif opponent_strategy == 'LLM':
                #insert previous moves but swapping the assignment - treat agentO as lead agent 
                query_text2 = generate_IPD_query(prev_move_opp, prev_move_M, model_name, C_placeholder, D_placeholder, RN_stream_1, RN_stream_2)
                #print('\n prev_move_M at start of batch step: ', prev_move_M)
                print_cuda_mem_usage(device, episode, toprint=f'  MEM usage during batch step {batch_step}, after generate_IPD_query2: ')

                # Tokenize input query
                query_input_tensors2 = tokenizer(query_text2, return_tensors="pt", add_special_tokens=False)['input_ids'].to(device)
                print_cuda_mem_usage(device, episode, toprint=f'  MEM usage during batch step {batch_step}, after tokenizing IPD_query2: ')

                response_tensor2 = respond_to_batch(model2, query_input_tensors2, txt_len=gen_len, top_k = 0, top_p = top_p)
                print_cuda_mem_usage(device, episode, toprint=f'  MEM usage during batch step {batch_step}, after getting response from model2: ')
                response_text2 = tokenizer.decode(response_tensor2.squeeze(0), skip_special_tokens=True)
                response_text_withspecial2 = tokenizer.decode(response_tensor2.squeeze(0), skip_special_tokens=False)
                if episode == 0:
                    print('response_tensor2: ', response_tensor2)
                    print('response_text_withspecialtokens2, squeeze: ', response_text_withspecial2)
            ...
            if episode == 0:
                print('action_playerO (skip special tokens & strip): ', action_playerO, '\n')
            ...
            if action_playerM in [C_str, D_str]:
                action_for_payoff_M = action_playerM
            else:
                action_for_payoff_M = prev_move_M
            # Compute reward
            reward_M = reward_fn_game(action_playerM, action_for_payoff_O, C_str, D_str)#.to(device)
            reward_O = reward_fn_game(action_playerO, action_for_payoff_M, C_str, D_str)#.to(device)
            print_cuda_mem_usage(device, episode, toprint=f'  MEM usage during batch step {batch_step}, after calulating reward_game for players M and O: ')
            
            if action_playerM in [C_str, D_str]: #if M played a legal token on this turn 
                action_to_record_M = action_playerM
            else: 
                action_to_record_M = 'illegal'

            if action_playerO in [C_str, D_str]: #if O played a legal token on this turn
                action_to_record_O = action_playerO
            else:
                action_to_record_O = 'illegal'

            #record actions etc. in results_df
            results_df.loc[results_idx] = ['PT2 (Game reward)', episode, batch_step, str(C_str), str(D_str), prev_move_M, prev_move_opp, action_to_record_M, action_to_record_O, reward_M.numpy().item(), reward_O.numpy().item()]
            ...
            batch_queries.append(query_text1)
            batch_input_tensors_squeezed.append(query_input_tensors1.squeeze(0))
            batch_responses_squeezed.append(response_tensor1.squeeze(0))
            batch_responses_cpu.append(str(response_tensor1.squeeze(0).cpu().detach().numpy()))
            batch_responses_text.append(response_text1)
            batch_responses_text_withspecial.append(response_text_withspecial1)
            batch_opp_moves.append(action_playerO)
            batch_rewards.append(reward_M)#.numpy())
            print_cuda_mem_usage(device, episode, toprint=f'  MEM usage at the end of batch_step {batch_step}, after appending data for player M: ')

            if opponent_strategy == 'LLM':
                batch_queries2.append(query_text2)
                batch_input_tensors_squeezed2.append(query_input_tensors2.squeeze(0))
                batch_responses_squeezed2.append(response_tensor2.squeeze(0))
                batch_responses_cpu2.append(str(response_tensor2.squeeze(0).cpu().detach().numpy()))
                batch_responses_text2.append(response_text2)
                batch_responses_text_withspecial2.append(response_text_withspecial2)
                batch_opp_moves2.append(action_playerM)
            batch_rewards2.append(reward_O)#.numpy())
            print_cuda_mem_usage(device, episode, toprint=f'  MEM usage at the end of batch_step {batch_step}, after appending data for player O: ')

        batch_rewards_values1 = [t.numpy().item() for t in batch_rewards]
        batch_rewards_values2 = [t.numpy().item() for t in batch_rewards2]
        ...
        print('Rewards: ', batch_rewards_values1)

        print_cuda_mem_usage(device, episode, toprint='  MEM usage before ppo_trainer.step: ')

        stats1 = ppo_trainer.step(batch_input_tensors_squeezed, batch_responses_squeezed, batch_rewards)
        print_cuda_mem_usage(device, episode, toprint='  MEM usage after ppo_trainer.step: ')
        
        batch = {'query': batch_queries, 'opponent_prev_move':batch_opp_moves, 'response_ids':batch_responses_cpu, 'response':batch_responses_text_withspecial, 'fine-tuning phase': 'PT2 - Game rewards (player1)'}

        try: 
            ppo_trainer.log_stats(stats1, pd.DataFrame(batch), batch_rewards_values1, columns_to_log=['fine-tuning phase', 'query', 'opponent_prev_move', 'response_ids', 'response'])
        except Exception as e: 
            print('could not log desired columns to wandb for player1')
            ppo_trainer.log_stats(stats1, pd.DataFrame(batch), batch_rewards_values1)

        if opponent_strategy == 'LLM':
            print('Responses2 with special tokens: ', batch_responses_text_withspecial2)
            print('Responses2: ', batch_responses_text2)
            print('Opp. moves2: ', batch_opp_moves2)
            print('Rewards2: ', batch_rewards_values2)

            stats2 = ppo_trainer2.step(batch_input_tensors_squeezed2, batch_responses_squeezed2, batch_rewards2)
            print_cuda_mem_usage(device, episode, toprint='  MEM usage after ppo_trainer2.step: ')

            batch2 = {'query': batch_queries2, 'opponent_prev_move':batch_opp_moves2, 'response_ids':batch_responses_cpu2, 'response':batch_responses_text_withspecial2, 'fine-tuning phase': 'PT2 - Game rewards (player2)'}

            try: 
                ppo_trainer2.log_stats(stats2, pd.DataFrame(batch2), batch_rewards_values2, columns_to_log=['fine-tuning phase', 'query', 'opponent_prev_move', 'response_ids', 'response'])
            except Exception as e: 
                print('could not log desired columns to wandb for player2')
    ...
    results_df.to_csv(f'{OUTPUT_DIR}/During FT PART2 (Game rewards).csv') #this will overwrite previous files 
    print(results_df[['episode', 'action_M', 'action_O', 'reward_M', 'reward_O']].head())
    print(results_df[['episode', 'action_M', 'action_O', 'reward_M', 'reward_O']].tail())
    ...
    ######################################
    if save_model_pt2:
        new_model_dir = f"{HF_HOME}/{model_name}_FT_PT2_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentM"
        ...
        print(f'successfully saved agentM models and tokenizer to disc, in {new_model_dir}')

        if opponent_strategy == 'LLM': 
            new_model2_dir = f"{HF_HOME}/{model_name}_FT_PT2_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentO"
            if not os.path.exists(new_model2_dir):
                os.makedirs(new_model2_dir)
            ppo_trainer2.model.save_pretrained(new_model2_dir, push_to_hub=False)
            ppo_trainer2.tokenizer.save_pretrained(new_model2_dir, push_to_hub=False)
            print(f'successfully saved agentO models and tokenizer to disc, in {new_model2_dir}')


    elif save_model_pt2before3: 
        new_model_dir = f"{HF_HOME}/{model_name}_FT_PT2before3_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentM"
        if not os.path.exists(new_model_dir):
            os.makedirs(new_model_dir)
        ppo_trainer.model.save_pretrained(new_model_dir, push_to_hub=False)
        ppo_trainer.tokenizer.save_pretrained(new_model_dir, push_to_hub=False)
        ...
        if opponent_strategy == 'LLM':
            new_model2_dir = f"{HF_HOME}/{model_name}_FT_PT2before3_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentO"
            if not os.path.exists(new_model2_dir):
                os.makedirs(new_model2_dir)
            ppo_trainer2.model.save_pretrained(new_model2_dir, push_to_hub=False)
            ppo_trainer2.tokenizer.save_pretrained(new_model2_dir, push_to_hub=False)
    ...
    response_tensors_ref, response_tensors1, response_tensors2 = [], [], []
    response_texts_ref, response_texts1, response_texts2 = [], [], []
    opponent_moves1, opponent_moves2 = [], []
    rewards_before = []
    rewards_after1, rewards_after2 = [], []
    ...
    for i in range(BATCH_SIZE_eval):
        ...
        if i == 0: 
            with open (f"{OUTPUT_DIR}/IPD_eval_prompt_iter1_2agents.txt", 'w') as f: 
                f.write('query for agentM: \n')
                f.write(query_text1)
                if opponent_strategy=='LLM':
                    f.write('\nquery for agentO: \n')
                    f.write(query_text2)
        ...
        response_tensor_ref = respond_to_batch(ref_model, query_input_tensors1, txt_len=gen_len, top_k = 0, top_p = 1.0) 
        response_text_ref = tokenizer.decode(response_tensor_ref.squeeze(0), skip_special_tokens=True) #[-gen_len:]
        response_texts_ref.append(response_text_ref)

        action_player_ref = get_action_token_v3(response_text_ref)#[0]

        # Compute reward
        reward_ref = reward_fn_game(action_playerM, action_playerO, C_str, D_str)#.to(device)
        rewards_before.append(reward_ref.numpy().item())


        # Get response from fine-tuned model 1
        ...
        if opponent_strategy == 'TFT': #if fixed strategy, it will use the opponent object from above 
            opponent_move = opponent.make_move(prev_move_opponent=prev_move_M) #NB this returns a string 
        elif opponent_strategy in ['AC', 'AD', 'Random']: #if opponent follows any other strategy
            opponent_move = opponent.make_move(prev_move_opponent=None)
        elif opponent_strategy == 'LLM': #if LLM opponent, it will use the model2 object from above 
            # Tokenize query 
            query_input_tensors2 = tokenizer(query_text2, return_tensors="pt", add_special_tokens=False)['input_ids'].to(device)
            # Get response from fine-tuned model 2
            response_tensor2 = respond_to_batch(model2, query_input_tensors2, txt_len=gen_len, top_k = 0, top_p = top_p)
            response_text2 = tokenizer.decode(response_tensor2.squeeze(0), skip_special_tokens=True)
        ...
        opponent_moves1.append(opponent_move)
        opponent_moves2.append(action_playerM)

        # Compute reward
        reward_M = reward_fn_game(action_playerM, action_playerO, C_str, D_str)#.to(device)
        reward_O = reward_fn_game(action_playerO, action_playerM, C_str, D_str)#.to(device)

        rewards_after1.append(reward_M.numpy().item())
        rewards_after2.append(reward_O.numpy().item())
    ...
    print_cuda_mem_usage(device, episode=0, toprint='  MEM usage after evaluating fine-tuned models M and O: ')

    #### store decoded queries & responses
    game_data_pt2["query_agentM"] = queries_text1 

    game_data_pt2["response (before)"] = response_texts_ref 
    game_data_pt2["response (after) - agentM"] = response_texts1

    if opponent_strategy == 'LLM':
        game_data_pt2["query_agentO"] = queries_text2 
        game_data_pt2["response (after) - agentO"] = response_texts2

    game_data_pt2["opponent_move_agentM"] = opponent_moves1
    game_data_pt2["opponent_move_agentO"] = opponent_moves2

    game_data_pt2["rewards_Game (before)"] = rewards_before
    game_data_pt2["rewards_Game (after) - agentM"] = rewards_after1
    game_data_pt2["rewards_Game (after) - agentO"] = rewards_after2

    # store results in a dataframe
    df_results_pt2 = pd.DataFrame(game_data_pt2)
    #, columns=['query', 'opponent_move', 'response (before)', 'response (after) - agentM', 'response (after) - agentO', 'rewards_Game (before)', 'rewards_Game (after) - agentM', 'rewards_game (after) - agentO' ])
    print(df_results_pt2[['response (before)', 'response (after) - agentM', 'rewards_Game (before)', 'rewards_Game (after) - agentM', 'rewards_Game (after) - agentO']].head(10)) # 'response (after) - agentO',
    print(df_results_pt2['response (after) - agentM'].value_counts())
    if opponent_strategy == 'LLM':
        print(df_results_pt2['response (after) - agentO'].value_counts())
...
if do_PART3: 
    ...
    for episode in range(num_episodes): 
        ...
        for batch_step in range(max_steps_per_episode):
            ...
            if episode == 0: 
                print('query_text1 (before insert_CD_symbols): \n', query_text1)
            print('\n prev_move_M at start of batch step: ', prev_move_M)
            print('\n prev_move_opp at start of batch step: ', prev_move_opp)
            ...
            # Compute reward
            if moral_type == 'De': 
                reward_M = reward_fn_De(action_playerM, prev_move_opp, C_str, D_str)
                reward_O = reward_fn_De(action_playerO, prev_move_M, C_str, D_str)
            elif moral_type == 'Ut': 
                reward_M = reward_fn_Ut(action_playerM, action_playerO, C_str, D_str)
                reward_O = reward_fn_Ut(action_playerO, action_playerM, C_str, D_str)

            #record actions etc. in results_df
            results_df.loc[results_idx] = [f'PT3 ({moral_type} reward)', episode, batch_step, str(C_str), str(D_str), prev_move_M, prev_move_opp, action_to_record_M, action_to_record_O, reward_M.numpy().item(), reward_O.numpy().item()]
            ...
            if opponent_strategy == 'LLM':
                ...
                batch_responses_text_withspecial2.append(response_text_withspecial2)
                batch_opp_moves2.append(action_playerM)
            batch_rewards2.append(reward_O)
        ...
        stats1 = ppo_trainer.step(batch_input_tensors_squeezed, batch_responses_squeezed, batch_rewards)        
        batch = {'query': batch_queries, 'opponent_prev_move':batch_opp_moves, 'response_ids':batch_responses_cpu, 'response':batch_responses_text_withspecial, 'fine-tuning phase': f'PT3 - {moral_type} rewards (player1)'}
        try:
            ppo_trainer.log_stats(stats1, pd.DataFrame(batch), batch_rewards_values1, columns_to_log=['fine-tuning phase', 'query', 'opponent_prev_move', 'response_ids', 'response'])
        except Exception as e: 
            print('could not log desired columns to wandb for player1')
        ...
        if opponent_strategy == 'LLM':
            ...
            stats2 = ppo_trainer2.step(batch_input_tensors_squeezed2, batch_responses_squeezed2, batch_rewards2)
            batch2 = {'query': batch_queries2, 'opponent_prev_move':batch_opp_moves2, 'response_ids':batch_responses_cpu2, 'response':batch_responses_text_withspecial2, 'fine-tuning phase': f'PT3 - {moral_type} rewards (player2)'}
            try: 
                ppo_trainer2.log_stats(stats2, pd.DataFrame(batch2), batch_rewards_values2, columns_to_log=['fine-tuning phase', 'query', 'opponent_prev_move', 'response_ids', 'response'])
            except Exception as e: 
                print('could not log desired columns to wandb for player2')
    ...
    results_df.to_csv(f'{OUTPUT_DIR}/During FT PART3 ({moral_type} rewards).csv') #this will overwrite previous files 
    print(results_df[['episode', 'action_M', 'action_O', 'prev_move_M', 'prev_move_O', 'reward_M', 'reward_O']].head())
    print(results_df[['episode', 'action_M', 'action_O', 'prev_move_M', 'prev_move_O', 'reward_M', 'reward_O']].tail())
    ...
    ######################################
    if save_model_pt3:
        new_model_dir = f"{HF_HOME}/{model_name}_FT_PT3_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentM"
        if not os.path.exists(new_model_dir):
            os.makedirs(new_model_dir)
        ppo_trainer.model.save_pretrained(new_model_dir, push_to_hub=False)
        ppo_trainer.tokenizer.save_pretrained(new_model_dir, push_to_hub=False)
        ...
        if opponent_strategy == 'LLM':
            new_model2_dir = f"{HF_HOME}/{model_name}_FT_PT3_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentO"
            if not os.path.exists(new_model2_dir):
                os.makedirs(new_model2_dir)
            ppo_trainer2.model.save_pretrained(new_model2_dir, push_to_hub=False)
            ppo_trainer2.tokenizer.save_pretrained(new_model2_dir, push_to_hub=False)
        ...
        new_model_dir = f"{HF_HOME}/{model_name}_FT_PT3after2_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentM"
        if not os.path.exists(new_model_dir):
            os.makedirs(new_model_dir)
        ppo_trainer.model.save_pretrained(new_model_dir, push_to_hub=False)
        ppo_trainer.tokenizer.save_pretrained(new_model_dir, push_to_hub=False)
        ...
        if opponent_strategy == 'LLM':
            new_model2_dir = f"{HF_HOME}/{model_name}_FT_PT3after2_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentO"
            if not os.path.exists(new_model2_dir):
                os.makedirs(new_model2_dir)
            ppo_trainer2.model.save_pretrained(new_model2_dir, push_to_hub=False)
            ppo_trainer2.tokenizer.save_pretrained(new_model2_dir, push_to_hub=False)
    ...
    rewards_before = []
    rewards_after1, rewards_after2 = [], []
    rewards_game_before, rewards_game_after_M, rewards_game_after_O = [], [], []
    prev_moves_M, prev_moves_O = [], []
    ...
    for i in range(BATCH_SIZE_eval):
        ...
        if i == 0: 
            with open (f"{OUTPUT_DIR}/IPD_eval_prompt_iter1_2agents.txt", 'w') as f: 
                ...
                f.write('\nquery for agentO: \n')
                f.write(query_text2)
                f.write('\nprev_move_M, prev_move_O: \n')
                f.write(f'{prev_move_M}, {prev_move_opp}')
                f.close()
        ...
        # Get response from ref model
        response_tensor_ref = respond_to_batch(ref_model, query_input_tensors1, txt_len=gen_len, top_k = 0, top_p = 1.0) 
        response_text_ref = tokenizer.decode(response_tensor_ref.squeeze(0), skip_special_tokens=True)
        response_texts_ref.append(response_text_ref)

        action_player_ref = get_action_token_v3(response_text_ref)

        # Compute reward for ref model 
        if moral_type == 'De': 
            reward_ref = reward_fn_De(action_player_ref, prev_move_opp, C_str, D_str)
        elif moral_type == 'Ut':
            reward_ref = reward_fn_Ut(action_player_ref, action_playerO, C_str, D_str)
        rewards_before.append(reward_ref.numpy().item())

        reward_game_before = reward_fn_game(action_player_ref, action_playerO, C_str, D_str)#.to(device)
        ...
        # Compute reward for players M and O 
        if moral_type == 'De': 
            reward_M = reward_fn_De(action_playerM, prev_move_opp, C_str, D_str)
            reward_O = reward_fn_De(action_playerO, prev_move_M, C_str, D_str)
        elif moral_type == 'Ut':
        ...
        reward_game_M = reward_fn_game(action_playerM, action_playerO, C_str, D_str)
        rewards_game_after_M.append(reward_game_M.numpy().item())
        reward_game_O = reward_fn_game(action_playerO, action_playerM, C_str, D_str)#.to(device)
        rewards_game_after_O.append(reward_game_O.numpy().item())

        #record prev_move for players M and O for storing results 
        prev_moves_M.append(prev_move_M)
    ...
    #### store decoded queries & responses
    game_data_pt3["query_agentM"] = queries_text1 

    game_data_pt3["response (before)"] = response_texts_ref 
    game_data_pt3["response (after) - agentM"] = response_texts1

    game_data_pt3["opponent_move_agentM"] = opponent_moves1
    game_data_pt3["opponent_move_agentO"] = opponent_moves2

    game_data_pt3['prev_move - agentM'] = prev_moves_M
    game_data_pt3['prev_move - agentO'] = prev_moves_O

    game_data_pt3["rewards_De (before)"] = rewards_before
    game_data_pt3["rewards_De (after) - agentM"] = rewards_after1
    game_data_pt3["rewards_De (after) - agentO"] = rewards_after2

    game_data_pt3["rewards_Game (before)"] = rewards_game_before
    game_data_pt3["rewards_Game (after) - agent M"] = rewards_game_after_M
    game_data_pt3["rewards_Game (after) - agent O"] = rewards_game_after_O


    if opponent_strategy == 'LLM':
        game_data_pt3["query_agentO"] = queries_text2 
        game_data_pt3["response (after) - agentO"] = response_texts2 


    # store results in a dataframe
    df_results_pt3 = pd.DataFrame(game_data_pt3)
    #, columns=['query', 'opponent_move', 'response (before)', 'response (after) - agentM', 'response (after) - agentO', 'rewards_Game (before)', 'rewards_Game (after) - agentM', 'rewards_game (after) - agentO' ])
    print(df_results_pt3[['response (before)', 'response (after) - agentM', f'rewards_{moral_type} (before)', 'prev_move - agentM', 'prev_move - agentO', f'rewards_{moral_type} (after) - agentM', f'rewards_{moral_type} (after) - agentO']].head(10))
    print(df_results_pt3['response (after) - agentM'].value_counts())
    if opponent_strategy == 'LLM':
        print(df_results_pt3['response (after) - agentO'].value_counts())
...
if do_PART4: 
    ...
    for episode in range(num_episodes): 
        ...
        for batch_step in range(max_steps_per_episode):
            ...
            # Compute reward
            reward_M = reward_fn_gameDe(action_playerM, action_playerO, prev_move_opp, C_str, D_str)
            reward_O = reward_fn_gameDe(action_playerO, action_playerM, prev_move_M, C_str, D_str)


            #record actions etc. in results_df
            results_df.loc[results_idx] = ['PT4 (De&Game reward)', episode, batch_step, str(C_str), str(D_str), prev_move_M, prev_move_opp, action_to_record_M, action_to_record_O, reward_M.numpy().item(), reward_O.numpy().item()]
        ...
        stats1 = ppo_trainer.step(batch_input_tensors_squeezed, batch_responses_squeezed, batch_rewards)
        batch = {'query': batch_queries, 'opponent_prev_move':batch_opp_moves, 'response_ids':batch_responses_cpu, 'response':batch_responses_text_withspecial, 'fine-tuning phase': 'PT4 - De&Game rewards (player1)'}
        ...
        if opponent_strategy == 'LLM':
            ...
            stats2 = ppo_trainer2.step(batch_input_tensors_squeezed2, batch_responses_squeezed2, batch_rewards2)
            batch2 = {'query': batch_queries2, 'opponent_prev_move':batch_opp_moves2, 'response_ids':batch_responses_cpu2, 'response':batch_responses_text_withspecial2, 'fine-tuning phase': 'PT4 - De&Game rewards (player2)'}

            try: 
                ppo_trainer2.log_stats(stats2, pd.DataFrame(batch2), batch_rewards_values2, columns_to_log=['fine-tuning phase', 'query', 'opponent_prev_move', 'response_ids', 'response'])
            except Exception as e: 
                print('could not log desired columns to wandb for player2')
                ppo_trainer2.log_stats(stats2, pd.DataFrame(batch2), batch_rewards_values2)

        if episode % 100 == 0:
            results_df.to_csv(f'{OUTPUT_DIR}/During FT PART4 (De&Game rewards).csv') #this will overwrite previous files 


    print('SUCCESS in running PART 4 (De&Game fine-tuning) training loop')
    ...
    ######################################
    if save_model_pt4:
        new_model_dir = f"{HF_HOME}/{model_name}_FT_PT4_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentM"
        if not os.path.exists(new_model_dir):
            os.makedirs(new_model_dir)
        ppo_trainer.model.save_pretrained(new_model_dir, push_to_hub=False)
        ppo_trainer.tokenizer.save_pretrained(new_model_dir, push_to_hub=False)
        ...
        if opponent_strategy == 'LLM':
            new_model2_dir = f"{HF_HOME}/{model_name}_FT_PT4_opp{opponent_strategy}_run{run_idx}_{num_episodes}ep{OPTION}_agentO"
            if not os.path.exists(new_model2_dir):
                os.makedirs(new_model2_dir)
            ppo_trainer2.model.save_pretrained(new_model2_dir, push_to_hub=False)
            ppo_trainer2.tokenizer.save_pretrained(new_model2_dir, push_to_hub=False)
    ...
    for i in range(BATCH_SIZE_eval):
        ...
        # Compute reward
        reward_ref = reward_fn_gameDe(action_player_ref, action_playerO, prev_move_opp, C_str, D_str)
        rewards_before.append(reward_ref.numpy().item())
        ...
        reward_game_M = reward_fn_game(action_playerM, action_playerO, C_str, D_str)
        rewards_game_after_M.append(reward_game_M.numpy().item())
        reward_game_O = reward_fn_game(action_playerO, action_playerM, C_str, D_str)
        rewards_game_after_O.append(reward_game_O.numpy().item())
    ...
    #### store decoded queries & responses
    game_data_pt4["query_agentM"] = queries_text1 
    
    game_data_pt4["response (before)"] = response_texts_ref 
    game_data_pt4["response (after) - agentM"] = response_texts1

    if opponent_strategy == 'LLM':
        game_data_pt4["query_agentO"] = queries_text2 
        game_data_pt4["response (after) - agentO"] = response_texts2 

    game_data_pt4["opponent_move_agentM"] = opponent_moves1
    game_data_pt4["opponent_move_agentO"] = opponent_moves2

    game_data_pt4['prev_move - agentM'] = prev_moves_M
    game_data_pt4['prev_move - agentO'] = prev_moves_O

    game_data_pt4["rewards_De&Game (before)"] = rewards_before
    game_data_pt4["rewards_De&Game (after) - agentM"] = rewards_after1
    game_data_pt4["rewards_De&Game (after) - agentO"] = rewards_after2

    game_data_pt4["rewards_Game (before)"] = rewards_game_before
    game_data_pt4["rewards_Game (after) - agent M"] = rewards_game_after_M
    game_data_pt4["rewards_Game (after) - agent O"] = rewards_game_after_O

    # store results in a dataframe
    df_results_pt4 = pd.DataFrame(game_data_pt4)
    #, columns=['query', 'opponent_move', 'response (before)', 'response (after) - agentM', 'response (after) - agentO', 'rewards_Game (before)', 'rewards_Game (after) - agentM', 'rewards_game (after) - agentO' ])
    print(df_results_pt4[['response (before)', 'response (after) - agentM', 'response (after) - agentO', 'rewards_De&Game (before)', 'prev_move - agentM', 'prev_move - agentO', 'rewards_De&Game (after) - agentM', 'rewards_De&Game (after) - agentO']].head(10))
    print(df_results_pt4['response (after) - agentM'].value_counts())
    if opponent_strategy == 'LLM':
        print(df_results_pt4['response (after) - agentO'].value_counts())
